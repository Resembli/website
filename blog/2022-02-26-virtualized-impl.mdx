...
[`@resembli/react-virtualized-window`](http://localhost:3000/docs/react-virtualized-window/). If you found
it useful, consider giving our main repo a star: [Resembli MonoRepo](https://github.com/Resembli/ui).

:::

## What is [`@resembli/react-virtualized-window`](http://localhost:3000/docs/react-virtualized-window/)
...
- The total height and width needed to render all the items in the container (i.e. how much space would be needed if we didn't virtualize the view)
...
Then your container dimensions have a height of `500px` and a width of `500px`. Straightforward enough.

Now we need to compute the total height and width needed if we wanted to render every single item in our
window. For example, if we have a list of 1000 items, and each item takes `20px` height, then the total height
is `20,000px`. We can do the same for `width`.
...
Notice we have created a window with enough space to hold `20000px` of content. But now we need to know
...
- We know the scroll offset from the top is `2000px`
...
user scrolls a little, let say `10px`, it means one item will only be half in view, leaving a little space at the bottom where
...
individual items we render a single large `div` with the height of all the individual items combined. That's it!
...
work. The advantage of using a `div` is that we can utilize the browsers normal capabilities for laying out
items, instead of individually positioning each item.
...
powerful enough things may not be as smooth as they could without the `sticky` positioning. Furthermore, `sticky` positioning as a few
